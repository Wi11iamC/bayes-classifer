...
def perform_training(X_train, training_labels, X_test, test_labels):
  final_result = []
  ...
  for i in range(10):
    ...
    # set max k value for smoothing paramter
    k_max = 30
    # initialize k, accuracy, and prediction lists for finding optimal smoothing paramter value
    gnb_accuracy_list = np.zeros(k_max)
    gnb_prediction_list = np.zeros((k_max, len(test_labels)))
    k = np.zeros(k_max, dtype=np.float64)
    training_elapsed_time = np.zeros(k_max, dtype=np.float64)
    testing_elapsed_time = np.zeros(k_max, dtype=np.float64)
    fixed_smoothing_param = 0.004
    for j in range(k_max):
      gnb = GaussianNaiveBayes()
      k[j] = 1 / pow(10, j)
      # Training...
      training_start_time = time.time()
      gnb.train(sampled_X_train, sampled_training_labels, 1 / pow(10, j), fixed_smoothing_term=fixed_smoothing_param)
      training_end_time = time.time()
      training_elapsed_time[j] = training_end_time - training_start_time
      # Testing...
      testing_start_time = time.time()
      gnb_prediction_list[j] = gnb.predict(X=X_test)
      testing_end_time = time.time()
      testing_elapsed_time[j] = testing_end_time - testing_start_time
      # Computing Accuracy...
      gnb_accuracy_list[j] = accuracy(test_labels, gnb_prediction_list[j])
    # Finding optimal smoothing paramter
    optimal_k = np.argmax(gnb_accuracy_list)
    # Generating Confusion Matrix
    confusion_matrix = metrics.confusion_matrix(test_labels, gnb_prediction_list[optimal_k])
    # Generate data to append to final_result
    sampled_res = {'title': str((i + 1) *10) + '% of data', 'percentage_of_data': int(i+1) / 10, 'optimal_smoothing_parameter': np.float64(k[optimal_k]), 'optimal_k': optimal_k, 'training_elapsed_time': training_elapsed_time[optimal_k], 'testing_elapsed_time': testing_elapsed_time[optimal_k], 'best_prediction_list': gnb_prediction_list[optimal_k], 'fixed_smoothing_term': np.float64(fixed_smoothing_param), 'accuracy': gnb_accuracy_list[optimal_k], 'confusion_matrix': confusion_matrix}
